...
{
    public class UnassgnClaimsNITE_URController : BaseController
    {
        #region Search
        ...
        public ActionResult Search()
        {
            ...
            UnassignedClaimEASearchModel objSearchModel = new UnassignedClaimEASearchModel() { ClinicID = ArivaSession.Sessions().SelClinicID, StatusIDs = Convert.ToByte(ClaimStatus.EA_GENERAL_QUEUE_NOT_IN_TRACK).ToString() };
        ...
        public ActionResult Search(UnassignedClaimEASearchModel objSearchModel)
        {
            if (objSearchModel.CurrNumber > 0)
            {
                ...
                {
                    return ResponseDotRedirect("AssgnClaimsNITE", "Save", 0, 1);
                }
                else
                {
                    return ResponseDotRedirect("UnassgnClaimsNITE", "Search", 0, 101);
                }
            }

            objSearchModel.ClinicID = ArivaSession.Sessions().SelClinicID;
            objSearchModel.StatusIDs = Convert.ToByte(ClaimStatus.EA_GENERAL_QUEUE_NOT_IN_TRACK).ToString();
            //objSearchModel.Fill(IsActive());
            return ResponseDotRedirect(objSearchModel);
        }
        ...
        public ActionResult View(UnassignedClaimEASearchModel objSearchModel)
        {
            if (objSearchModel.CurrNumber > 0)
            {
                ...
                return ResponseDotRedirect("UnassgnClaimsNITE", "Save", 0, 1);
            }
        ...
        {
            UnassignedClaimSearchModel objSearchModel = new UnassignedClaimSearchModel()
            {
                ...
                ,
                statusIDs = Convert.ToByte(ClaimStatus.EA_GENERAL_QUEUE_NOT_IN_TRACK).ToString()
            };
