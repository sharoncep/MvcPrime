...
{
    public class UnassgnClaimsNITQ_URController : BaseController
    {
        #region Search
        ...
        public ActionResult Search()
        {
            ...
            UnassignedClaimQASearchModel objSearchModel = new UnassignedClaimQASearchModel() { ClinicID = ArivaSession.Sessions().SelClinicID, statusIDs = Convert.ToByte(ClaimStatus.QA_GENERAL_QUEUE_NOT_IN_TRACK).ToString() };
        ...
        public ActionResult Search(UnassignedClaimQASearchModel objSearchModel)
        {
            if (objSearchModel.CurrNumber > 0)
            {
                ...
                {
                    return ResponseDotRedirect("AssgnClaimsNITQ", "Save", 0, 1);
                }
                else
                {
                    return ResponseDotRedirect("UnassgnClaimsNITQ", "Search", 0, 101);
                }
            }

            objSearchModel.ClinicID = ArivaSession.Sessions().SelClinicID;
            objSearchModel.statusIDs = Convert.ToByte(ClaimStatus.QA_GENERAL_QUEUE_NOT_IN_TRACK).ToString();
            //objSearchModel.Fill(IsActive());
            return ResponseDotRedirect(objSearchModel);
        }
        ...
        public ActionResult View(UnassignedClaimQASearchModel objSearchModel)
        {
            if (objSearchModel.CurrNumber > 0)
            {
                ...
                return ResponseDotRedirect("UnassgnClaimsNITQ", "Save", 0, 1);
            }
        ...
        public ActionResult SearchAjaxCall(string pSearchName, string pDateFrom, string pDateTo, string pStartBy, string pOrderByField, string pOrderByDirection, string pCurrPageNumber)
        {
            UnassignedClaimSearchModel objSearchModel = new UnassignedClaimSearchModel()
            {
                ...
                ,
                statusIDs = Convert.ToByte(ClaimStatus.QA_GENERAL_QUEUE_NOT_IN_TRACK).ToString()
            };
