...
{
    public class CommunicationNumberQualifier_CURDController : BaseController
    {
        #region Search
        ...
        public ActionResult Search()
        {
            ...
            CommunicationNumberQualifierSearchModel objSearchModel = new CommunicationNumberQualifierSearchModel();
        ...
        public ActionResult Search(CommunicationNumberQualifierSearchModel objSearchModel)
        {
            if (objSearchModel.CurrNumber > 0)
            {
                ArivaSession.Sessions().PageEditID<global::System.Byte>(objSearchModel.CurrNumber);

                return ResponseDotRedirect("CommunicationNumberQualifier", "Save");
            }
        ...
        {
            CommunicationNumberQualifierSaveModel objModel = new CommunicationNumberQualifierSaveModel();
        ...
        public ActionResult Save(CommunicationNumberQualifierSaveModel objmodel)
        {
            bool saveTransactionTypeCode = objmodel.Save(ArivaSession.Sessions().UserID);
            if (saveTransactionTypeCode)
            {
                return ResponseDotRedirect("CommunicationNumberQualifier", "Search", 0, 1);
            }
            else
            {
