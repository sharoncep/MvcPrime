...
{
    public class EntityIdentifierCode_CURDController : BaseController
    {
        #region Search
        ...
        public ActionResult Search()
        {
            ...
            EntityIdentifierCodeSearchModel objSearchModel = new EntityIdentifierCodeSearchModel();
        ...
        public ActionResult Search(EntityIdentifierCodeSearchModel objSearchModel)
        {
            if (objSearchModel.CurrNumber > 0)
            {
                ArivaSession.Sessions().PageEditID<global::System.Byte>(objSearchModel.CurrNumber);

                return ResponseDotRedirect("EntityIdentifierCode", "Save");
            }
        ...
        {
            EntityIdentifierCodeSaveModel objModel = new EntityIdentifierCodeSaveModel();
        ...
        public ActionResult Save(EntityIdentifierCodeSaveModel objmodel)
        {
            bool saveTransactionTypeCode = objmodel.Save(ArivaSession.Sessions().UserID);
            if (saveTransactionTypeCode)
            {
                return ResponseDotRedirect("EntityIdentifierCode", "Search", 0, 1);
            }
            else
            {
