...
namespace ClaimatePrimeControllers.Controllers
{
    ...
    /// 
    /// </summary>
    public class AssgnClaimsQ_URController : BaseController
    {
        # region Constructors
        ...
        /// 
        /// </summary>
        public AssgnClaimsQ_URController()
        {
        }
        ...
        public ActionResult Search()
        {
            ...
            AssignedClaimQASearchModel objSearchModel = new AssignedClaimQASearchModel() { ClinicID = ArivaSession.Sessions().SelClinicID, statusIDs = string.Concat(Convert.ToByte(ClaimStatus.QA_PERSONAL_QUEUE), ", ", Convert.ToByte(ClaimStatus.QA_PERSONAL_QUEUE_NOT_IN_TRACK)), AssignedTo = ArivaSession.Sessions().UserID };
        ...
        public ActionResult Search(AssignedClaimQASearchModel objSearchModel)
        {
            if (objSearchModel.CurrNumber > 0)
            {
                ArivaSession.Sessions().PageEditID<global::System.Int64>(objSearchModel.CurrNumber);

                return ResponseDotRedirect("AssgnClaimsQ", "Save");
            }

            objSearchModel.ClinicID = ArivaSession.Sessions().SelClinicID;
            objSearchModel.statusIDs = string.Concat(Convert.ToByte(ClaimStatus.QA_PERSONAL_QUEUE), ", ", Convert.ToByte(ClaimStatus.QA_PERSONAL_QUEUE_NOT_IN_TRACK));
        ...
        public ActionResult Save()
        {
            ...
            AssignedClaimQASaveModel objSaveModel = new AssignedClaimQASaveModel();
        ...
        public ActionResult Save(AssignedClaimQASaveModel objSaveModel)
        {
            objSaveModel.PatientVisitID = ArivaSession.Sessions().PageEditID<long>();
            objSaveModel.CommentForReadyToSend = "Sys Gen : READY_TO_SEND_CLAIM";
            objSaveModel.CommentForEAGenQ = "Sys Gen : EA_GENERAL_QUEUE";
            objSaveModel.CommentForEAPerQ = "Sys Gen : EA_PERSONAL_QUEUE";

            if (objSaveModel.Save(ArivaSession.Sessions().UserID))
            {
                return ResponseDotRedirect("UnassgnClaimsQ", "Search", 0, 1);
            }
        ...
        [AcceptVerbs(HttpVerbs.Post)]
        [ActionName("Save")]
        [AcceptParameter(ButtonName = "btnReAsgn")]
        public ActionResult SaveHold(AssignedClaimQASaveModel objSaveModel)
        {
            objSaveModel.PatientVisitID = ArivaSession.Sessions().PageEditID<long>();
            objSaveModel.CommentForReAsgn = "Sys Gen : REASSIGNED_BY_QA_TO_BA";

            if (objSaveModel.ReassignSave(ArivaSession.Sessions().UserID))
            {
                return ResponseDotRedirect("UnassgnClaimsQ", "Search", 0, 1);
            }
