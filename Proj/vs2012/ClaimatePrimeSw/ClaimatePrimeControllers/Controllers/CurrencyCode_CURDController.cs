...
{
    public class CurrencyCode_CURDController : BaseController
    {
        #region Search
        ...
        public ActionResult Search()
        {
            ...
            CurrencyCodeSearchModel objSearchModel = new CurrencyCodeSearchModel();
        ...
        public ActionResult Search(CurrencyCodeSearchModel objSearchModel)
        {
            if (objSearchModel.CurrNumber > 0)
            {
                ArivaSession.Sessions().PageEditID<global::System.Byte>(objSearchModel.CurrNumber);

                return ResponseDotRedirect("CurrencyCode", "Save");
            }
        ...
        {
            CurrencyCodeSaveModel objModel = new CurrencyCodeSaveModel();
        ...
        public ActionResult Save(CurrencyCodeSaveModel objmodel)
        {
            bool saveTransactionTypeCode = objmodel.Save(ArivaSession.Sessions().UserID);
            if (saveTransactionTypeCode)
            {
                return ResponseDotRedirect("CurrencyCode", "Search", 0, 1);
            }
            else
            {
