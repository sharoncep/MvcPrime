...
{
    public class UnassgnClaims_URController : BaseController
    {
        #region Search
        ...
        public ActionResult Search()
        {
            ...
            UnassignedClaimSearchModel objSearchModel = new UnassignedClaimSearchModel()
            {
                ...
                ,
                statusIDs = string.Concat(Convert.ToByte(ClaimStatus.BA_GENERAL_QUEUE), ", ", Convert.ToByte(ClaimStatus.BA_GENERAL_QUEUE_NOT_IN_TRACK))
            };
        ...
        public ActionResult Search(UnassignedClaimSearchModel objSearchModel)
        {
            if (objSearchModel.CurrNumber > 0)
            {
                ArivaSession.Sessions().PageEditID<global::System.Int64>(objSearchModel.CurrNumber);
                objSearchModel._CommentForBAPerQ = "Sys Gen : BA_PERSONAL_QUEUE";

                if (objSearchModel.Save(ArivaSession.Sessions().UserID, IsActive(true)))
                {
                    return ResponseDotRedirect("AssgnClaims", "Save", 0, 1);
                }
                else
                {
                    return ResponseDotRedirect("UnassgnClaims", "Search", 0, 101);
                }
            }

            objSearchModel.ClinicID = ArivaSession.Sessions().SelClinicID;
            objSearchModel.statusIDs = string.Concat(Convert.ToByte(ClaimStatus.BA_GENERAL_QUEUE), ", ", Convert.ToByte(ClaimStatus.BA_GENERAL_QUEUE_NOT_IN_TRACK));
            //objSearchModel.Fill(IsActive());
            return ResponseDotRedirect(objSearchModel);
        }
        ...
        public ActionResult View(UnassignedClaimSearchModel objSearchModel)
        {
            if (objSearchModel.CurrNumber > 0)
            {
                ArivaSession.Sessions().PageEditID<global::System.Int64>(objSearchModel.CurrNumber);

                return ResponseDotRedirect("UnassgnClaims", "Save", 0, 1);
            }
