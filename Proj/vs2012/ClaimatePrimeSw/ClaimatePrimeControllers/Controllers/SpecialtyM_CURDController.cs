...
namespace ClaimatePrimeControllers.Controllers
{
    ...
    /// </summary>
    [Serializable]
    public class SpecialtyM_CURDController : BaseController
    {
        #region Search
        ...
        public ActionResult Search(SpecialtySearchModel objSearchModel)
        {
            if (objSearchModel.CurrNumber > 0)
            {
                ...
                return ResponseDotRedirect("SpecialtyM", "Save");
            }
        ...
        public ActionResult Save(SpecialtySaveModel objSaveModel)
        {
            //objSaveModel.Fill(ArivaSession.Sessions().UserID, IsActive());
            if (objSaveModel.Save(ArivaSession.Sessions().UserID))
            {

                return ResponseDotRedirect("SpecialtyM", "Search", objSaveModel.Specialty.SpecialtyName.Substring(0, 1), 1);
            }
            else
            {
