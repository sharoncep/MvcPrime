...
namespace ClaimatePrimeControllers.Controllers
{
    ...
    /// </summary>
    [Serializable]
    public class Country_URController : BaseController
    {
        #region Country
        ...
        public ActionResult Search()
        {
            ...
            CountrySearchModel objSearchModel = new CountrySearchModel();
        ...
        public ActionResult Search(CountrySearchModel objSearchModel)
        {
            if (objSearchModel.CurrNumber > 0)
            {
                ArivaSession.Sessions().PageEditID<byte>(objSearchModel.CurrNumber);

                return ResponseDotRedirect("Country", "Save");
            }
        ...
        {
            CountrySearchModel objSearchModel = new CountrySearchModel() { SearchName = pSearchName, StartBy = pStartBy, OrderByDirection = pOrderByDirection, OrderByField = pOrderByField };
            ...
            foreach (usp_GetBySearch_Country_Result item in objSearchModel.Countries)
            {
        ...
        public ActionResult BlockUnBlockAjaxCall(string pID, string pKy)
        {
            ...
            CountrySaveModel objSaveModel = new CountrySaveModel();
            objSaveModel.Fill(Converts.AsInt64(pID), IsActive());
            objSaveModel.Country.IsActive = (string.Compare(pKy, "U", true) == 0) ? true : false;
            objSaveModel.Country.Comment = string.Concat("BlockUnBlockAjaxCall Operation: ", (string.Compare(pKy, "U", true) == 0) ? "UNBLOCKED" : "BLOCKED");
