...
{
    public class SummaryClinic_RController : BaseController
    {
        static string DESC;
        ...
        /// 
        /// </summary>
        public SummaryClinic_RController()
        {
        }
        ...
        public ActionResult Search()
        {
            ...
            ClinicWiseSummaryModel objSearchModel = new ClinicWiseSummaryModel() { UserID = ArivaSession.Sessions().UserID };
        ...
        public ActionResult Search(ClinicWiseSummaryModel objSearchModel)
        {
        ...
        public ActionResult Save()
        {
            ...
            CLINIC_ID = Request.QueryString["pClinicID"];

            ClinicWiseSummaryModel objSearchModel = new ClinicWiseSummaryModel();
            // objSearchModel.getClinicVisit(CLINIC_ID, DESC, DAY_COUNT);
            return ResponseDotRedirect(objSearchModel);
        }
        ...
        [ActionName("Save")]
        [AcceptParameter(ButtonName = "btnSearch")]
        public ActionResult Save(ClinicWiseSummaryModel objSearchModel)
        {
            if (objSearchModel.CurrNumber > 0)
            {
                ...
                return ResponseDotRedirect("SummaryClinic", "Display");
            }
            return ResponseDotRedirect(objSearchModel);
        }
        #endregion
        ...
        public ActionResult SearchAjaxCall(string pStartBy)
        {
            ...
            ClinicWiseSummaryModel objSearchModel = new ClinicWiseSummaryModel() { StartBy = pStartBy, UserID = ArivaSession.Sessions().UserID };
            objSearchModel.FillJs(pageNo, IsActive(true), rec);

            foreach (usp_GetBySearch_Clinic_Result item in objSearchModel.ClinicResult)
            {
        ...
        /// <returns></returns>
        [AcceptVerbs(HttpVerbs.Post)]
        public ActionResult SearchAjaxCallTableCount(string pClinicID)
        {
            ClinicWiseSummaryModel objDash = new ClinicWiseSummaryModel();
            List<DashTableResult> retDash = new List<DashTableResult>();

            objDash.GetResultClinic(Converts.AsInt32(pClinicID));

            foreach (usp_GetClinicWiseSummary_PatientVisit_Result item in objDash.ClinicDash)
            {
        ...
        /// <returns></returns>
        [AcceptVerbs(HttpVerbs.Post)]
        public ActionResult SearchAjaxCallTotTableCount(string pClinicID)
        {
            ClinicWiseSummaryModel objDash = new ClinicWiseSummaryModel();
            List<SearchResult> retDash = new List<SearchResult>();

            objDash.GetTotalCount(Converts.AsInt32(pClinicID));
        ...
        public ActionResult SearchListAjaxCall(string pSearchName, string pDateFrom, string pDateTo, string pStartBy, string pOrderByField, string pOrderByDirection, string pCurrPageNumber)
        {
            ...
            objSearchModel.getClinicVisit(CLINIC_ID, DESC, DAY_COUNT, Converts.AsInt64(pCurrPageNumber), StaticClass.ConfigurationGeneral.mSearchRecordPerPageID);

            List<SearchResult> retAns = new List<SearchResult>();
            foreach (usp_GetClinicWiseVisit_PatientVisit_Result item in objSearchModel.ClinicWiseVisit)
            {
                retAns.Add((SearchResult)item);
            }
        ...
        public ActionResult SavePdf()
        {
            ...
            string clinicID = Request.QueryString["pClinicID"];

            ClinicWiseSummaryModel objSearchModel = new ClinicWiseSummaryModel();
            objSearchModel.FillPdfVisit(Converts.AsInt32(clinicID), dayCount, desc);
        ...
        public ActionResult SaveExcel()
        {
            ...
            objEx.FillDashboard((DateTime.Now.AddSeconds(StaticClass.SvrTimeSecDiff)), StaticClass.CsResources("RptExlSubjClinicWise").Replace("[X]", desc.ToUpper()), StaticClass.GetDateStr(), StaticClass.GetTimeStr(), StaticClass.GetDateTimeStr(), ArivaSession.Sessions().UserDispName);
            ExcelFileDash.Create(xlsxPath, objEx, ExcelReportModel.GetDataClinicWise(Converts.AsInt32(clinicID), desc, dayCount));
