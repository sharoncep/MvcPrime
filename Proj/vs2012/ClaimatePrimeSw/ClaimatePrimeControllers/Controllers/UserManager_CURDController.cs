...
{
    public class UserManager_CURDController : BaseController
    {
        #region Search
        ...
        public ActionResult Search()
        {
            ...
            UserSearchModel objSearchModel = new UserSearchModel() { SelHighRoleID = Convert.ToByte(Role.MANAGER_ROLE_ID) };
        ...
        public ActionResult Search(UserSearchModel objSearchModel)
        {
            if (objSearchModel.CurrNumber > 0)
            {
                ...
                return ResponseDotRedirect("UserManager", "Save");
            }
            objSearchModel.SelHighRoleID = Convert.ToByte(Role.MANAGER_ROLE_ID);
            objSearchModel.FillCs(IsActive());
            return ResponseDotRedirect(objSearchModel);
        }
        ...
        {
            UserSearchModel objSearchModel = new UserSearchModel() { SearchName = pSearchName, StartBy = pStartBy, SelHighRoleID = Convert.ToByte(Role.MANAGER_ROLE_ID), OrderByDirection = pOrderByDirection, OrderByField = pOrderByField };
        ...
        public ActionResult Save(UserSaveModel objSaveModel, HttpPostedFileBase filUpload)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    objSaveModel.FileSvrUserPhotoPath = string.Concat(objSaveModel.FileSvrUserPhotoPath, @"\", "P_", objSaveModel.ObjParam("Patient").Value);
                    if (!(Directory.Exists(objSaveModel.FileSvrUserPhotoPath)))
                    {
                        Directory.CreateDirectory(objSaveModel.FileSvrUserPhotoPath);
                    }
            ...
            if (objSaveModel.Save(ArivaSession.Sessions().UserID))
            {
                ...
                if (!objSaveModel.BillingAgentChk)
                {
                    return ResponseDotRedirect("UserManager", "Search", objSaveModel.User.LastName.Substring(0, 1), 1);
                }
                else
                {
                    return ResponseDotRedirect("UserA", "Search", objSaveModel.User.LastName.Substring(0, 1), 1);
                }
            }
