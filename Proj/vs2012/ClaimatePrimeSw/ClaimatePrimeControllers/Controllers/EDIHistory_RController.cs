...
{
    public class EDIHistory_RController : BaseController
    {
        #region Search
        ...
        public ActionResult Search()
        {
            ...
            EDIFileSearchModel objSearchModel = new EDIFileSearchModel();
        ...
        public ActionResult SearchAjaxCall(string pSearchName, string pDateFrom, string pDateTo, string pStartBy, string pOrderByField, string pOrderByDirection, string pCurrPageNumber)
        {
            ...
            foreach (usp_GetBySearch_EDIFile_Result item in objSearchModel.EDIFileResults)
            {
                EDIFileSearchSubModel objSubModel = new EDIFileSearchSubModel() { EDIFileID = item.EDIFileID, FileDate = StaticClass.GetDateTimeStr(item.CreatedOn), RefFileSvrPath = string.Concat(StaticClass.FileSvrRootPath, @"\", item.RefFileRelPath), X12FileSvrPath = string.Concat(StaticClass.FileSvrRootPath, @"\", item.X12FileRelPath) };
            ...
            foreach (EDIFileSearchSubModel item in objSearchModel.EDIFileSearchSubModels)
            {
        ...
        public ActionResult EDIView()
        {
            ...
            EDIFileSearchModel obj = new EDIFileSearchModel();

            EDIPatientVisitSearchModel objSearchModel = new EDIPatientVisitSearchModel() { EDIFileID = Convert.ToInt32(ArivaSession.Sessions().PageEditID<Int64>()), ClinicID = ArivaSession.Sessions().SelClinicID };
        ...
        [ActionName("EDIView")]
        [AcceptParameter(ButtonName = "btnSearch")]
        public ActionResult EDIView(EDIPatientVisitSearchModel objSearchModel)
        {
            if (objSearchModel.CurrNumber > 0)
            {
                ArivaSession.Sessions().PageEditID<global::System.Int64>(objSearchModel.CurrNumber);

                return ResponseDotRedirect("EDIHistory", "Save");
            }
            objSearchModel.EDIFileID = Convert.ToInt32(ArivaSession.Sessions().PageEditID<Int64>());
        ...
        /// <returns></returns>
        [AcceptVerbs(HttpVerbs.Post)]
        public ActionResult EDIViewAjaxCall(string pSearchName, string pStartBy, string pOrderByField, string pOrderByDirection, string pCurrPageNumber)
        {
            EDIFileSearchModel obj = new EDIFileSearchModel();
            EDIPatientVisitSearchModel objSearchModel = new EDIPatientVisitSearchModel() { SearchName = pSearchName, StartBy = pStartBy, OrderByDirection = pOrderByDirection, OrderByField = pOrderByField };
            objSearchModel.EDIFileID = Convert.ToInt32(ArivaSession.Sessions().PageEditID<Int64>());
            objSearchModel.ClinicID = ArivaSession.Sessions().SelClinicID;
            objSearchModel.FillJs(Converts.AsInt64(pCurrPageNumber), IsActive(), StaticClass.ConfigurationGeneral.mSearchRecordPerPageID);

            List<SearchResult> retAns = new List<SearchResult>();
            foreach (usp_GetByEDI_PatientVisit_Result item in objSearchModel.EDIPatientVisit)
            {
                retAns.Add((SearchResult)item);
            }
