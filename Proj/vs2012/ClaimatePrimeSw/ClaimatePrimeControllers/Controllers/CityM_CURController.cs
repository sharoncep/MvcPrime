...
{
    /// <summary>
    /// By Sai : Manager Role - City Edit/Save(No Block or Unblock)
    /// </summary>
    [Serializable]
    public class CityM_CURController : BaseController
    {
        #region Search
        ...
        public ActionResult Search(CitySearchModel objSearchModel)
        {
            if (objSearchModel.CurrNumber > 0)
            {
                ...
                return ResponseDotRedirect("CityM", "Save");
            }
        ...
        public ActionResult Save(CitySaveModel objSaveModel)
        {
            //objSaveModel.Fill(ArivaSession.Sessions().UserID, IsActive());
            if (objSaveModel.Save(ArivaSession.Sessions().UserID))
            {

                return ResponseDotRedirect("CityM", "Search", objSaveModel.City.CityName.Substring(0, 1), 1);
            }
