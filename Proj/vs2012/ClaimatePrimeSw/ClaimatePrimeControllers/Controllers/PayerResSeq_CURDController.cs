...
{
    public class PayerResSeq_CURDController : BaseController
    {
        #region Search
        ...
        public ActionResult Search()
        {
            ...
            PayerResponsibilitySequenceNumberCodeSearchModel objSearchModel = new PayerResponsibilitySequenceNumberCodeSearchModel();
        ...
        public ActionResult Search(PayerResponsibilitySequenceNumberCodeSearchModel objSearchModel)
        {
            if (objSearchModel.CurrNumber > 0)
            {
                ArivaSession.Sessions().PageEditID<global::System.Byte>(objSearchModel.CurrNumber);

                return ResponseDotRedirect("PayerResSeq", "Save");
            }
        ...
        {
            PayerResponsibilitySequenceNumberCodeSaveModel objModel = new PayerResponsibilitySequenceNumberCodeSaveModel();
        ...
        public ActionResult Save(PayerResponsibilitySequenceNumberCodeSaveModel objmodel)
        {
            bool saveTransactionTypeCode = objmodel.Save(ArivaSession.Sessions().UserID);
            if (saveTransactionTypeCode)
            {
                return ResponseDotRedirect("PayerResSeq", "Search", 0, 1);
            }
            else
            {
