...
{
    public class InterchangeIDQualifier_CURDController : BaseController
    {
        #region Search
        ...
        public ActionResult Search()
        {
            ...
            InterchangeIDQualifierSearchModel objSearchModel = new InterchangeIDQualifierSearchModel();
        ...
        public ActionResult Search(InterchangeIDQualifierSearchModel objSearchModel)
        {
            if (objSearchModel.CurrNumber > 0)
            {
                ArivaSession.Sessions().PageEditID<global::System.Byte>(objSearchModel.CurrNumber);

                return ResponseDotRedirect("InterchangeIDQualifier", "Save");
            }
        ...
        {
            InterchangeIDQualifierSaveModel objModel = new InterchangeIDQualifierSaveModel();
        ...
        public ActionResult Save(InterchangeIDQualifierSaveModel objmodel)
        {
            bool saveTransactionTypeCode = objmodel.Save(ArivaSession.Sessions().UserID);
            if (saveTransactionTypeCode)
            {
                return ResponseDotRedirect("InterchangeIDQualifier", "Search", 0, 1);
            }
            else
            {
