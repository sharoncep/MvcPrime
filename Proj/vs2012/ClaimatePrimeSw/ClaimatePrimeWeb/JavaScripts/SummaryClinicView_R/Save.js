    ...
    if (divTit != ky) {
        ...
        if (ky == "Cpt") {
            ...
            if (($("#FacilityDoneID").val().length > 0) && ($("#FacilityDoneID").val() > 0)) {
                ...
                setTimeout("expandPnlDiv('FacilityDone')", 700);
                setTimeout("collapsePnlDiv('Clinic')", 500);
...
function saveSuccess(data, status, ky) {
    if (data != null) {
        if (status.toLowerCase() == 'success') {
            var jsonData = data;
            if (jsonData != null) {
                if ((ky == "Visit") || (ky == "Dxprimary") || (ky == "Visitdoc") || (ky == "DrNote") || (ky == "SuperBill")) {
