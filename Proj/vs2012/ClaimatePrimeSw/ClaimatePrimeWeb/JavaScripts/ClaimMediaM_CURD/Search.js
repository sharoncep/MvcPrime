    ...
    if (($('#chkHasRec').is(':checked'))) {
        ...
        $.ajax({
            ...
            error: function (req, status, errorObj) {
                ajaxCallError("ClaimMediaM_CURD --> SearchAjaxCall", req, status, errorObj);
            }
        });
    }
...
function searchSuccess(data, status) {
    ...
    if (hml.length == 0) {
        ...
        hml = hml + "<a href=\"\" ";
        if ($("#OrderByField").val() == "ClaimMediaName") {
            if ($("#OrderByDirection").val() == "D") {
                hml = hml + "class=\"sort_descending\" title=\"";
                hml = hml + AlertMsgs.get("DECENDING");
                hml = hml + "\" onclick=\"javascript:return setSort('ClaimMediaName', 'A');\">";
            }
            else {
                hml = hml + "class=\"sort_acending\" title=\"";
                hml = hml + AlertMsgs.get("ACCENDING");
                hml = hml + "\" onclick=\"javascript:return setSort('ClaimMediaName', 'D');\">";
            }
        }
        else {
            hml = hml + "class=\"sort_normal\" onclick=\"javascript:return setSort('ClaimMediaName', 'A');\">";
        }
        ...
        hml = hml + "<td width=\"40%\" class=\"td-gridhead\">";
        hml = hml + "<a href=\"\" ";
        if ($("#OrderByField").val() == "ClaimMediaCode") {
            if ($("#OrderByDirection").val() == "D") {
                hml = hml + "class=\"sort_descending\" title=\"";
                hml = hml + AlertMsgs.get("DECENDING");
                hml = hml + "\" onclick=\"javascript:return setSort('ClaimMediaCode', 'A');\">";
            }
            else {
                hml = hml + "class=\"sort_acending\" title=\"";
                hml = hml + AlertMsgs.get("ACCENDING");
                hml = hml + "\" onclick=\"javascript:return setSort('ClaimMediaCode', 'D');\">";
            }
        }
        else {
            hml = hml + "class=\"sort_normal\" onclick=\"javascript:return setSort('ClaimMediaCode', 'A');\">";
        }
