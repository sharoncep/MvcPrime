    ...
    if (($('#chkHasRec').is(':checked'))) {
        ...
        $.ajax({
            ...
            error: function (req, status, errorObj) {
                ajaxCallError("CountyM_CUR --> SearchAjaxCall", req, status, errorObj);
            }
        });
    }
...
function searchSuccess(data, status) {
    ...
    if (hml.length == 0) {
        ...
        hml = hml + "<a href=\"\" ";
        if ($("#OrderByField").val() == "CountryName") {
            if ($("#OrderByDirection").val() == "D") {
                hml = hml + "class=\"sort_descending\" title=\"";
                hml = hml + AlertMsgs.get("DECENDING");
                hml = hml + "\" onclick=\"javascript:return setSort('CountryName', 'A');\">";
            }
            else {
                hml = hml + "class=\"sort_acending\" title=\"";
                hml = hml + AlertMsgs.get("ACCENDING");
                hml = hml + "\" onclick=\"javascript:return setSort('CountryName', 'D');\">";
            }
        }
        else {
            hml = hml + "class=\"sort_normal\" onclick=\"javascript:return setSort('CountryName', 'A');\">";
        }
        ...
        hml = hml + "<a href=\"\" ";
        if ($("#OrderByField").val() == "CountryCode") {
            if ($("#OrderByDirection").val() == "D") {
                hml = hml + "class=\"sort_descending\" title=\"";
                hml = hml + AlertMsgs.get("DECENDING");
                hml = hml + "\" onclick=\"javascript:return setSort('CountryCode', 'A');\">";
            }
            else {
                hml = hml + "class=\"sort_acending\" title=\"";
                hml = hml + AlertMsgs.get("ACCENDING");
                hml = hml + "\" onclick=\"javascript:return setSort('CountryCode', 'D');\">";
            }
        }
        else {
            hml = hml + "class=\"sort_normal\" onclick=\"javascript:return setSort('CountryCode', 'A');\">";
        }
