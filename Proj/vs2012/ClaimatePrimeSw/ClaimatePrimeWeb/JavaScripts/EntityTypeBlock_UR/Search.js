    ...
    if (($('#chkHasRec').is(':checked'))) {
        ...
        $.ajax({
            ...
            error: function (req, status, errorObj) {
                ajaxCallError("EntityTypeBlock_UR --> SearchAjaxCall", req, status, errorObj);
            }
        });
    }
...
function searchSuccess(data, status) {
    ...
    if (hml.length == 0) {
        ...
        hml = hml + "<a href=\"\" ";
        if ($("#OrderByField").val() == "EntityTypeName") {
            if ($("#OrderByDirection").val() == "D") {
                hml = hml + "class=\"sort_descending\" title=\"";
                hml = hml + AlertMsgs.get("DECENDING");
                hml = hml + "\" onclick=\"javascript:return setSort('EntityTypeName', 'A');\">";
            }
            else {
                hml = hml + "class=\"sort_acending\" title=\"";
                hml = hml + AlertMsgs.get("ACCENDING");
                hml = hml + "\" onclick=\"javascript:return setSort('EntityTypeName', 'D');\">";
            }
        }
        else {
            hml = hml + "class=\"sort_normal\" onclick=\"javascript:return setSort('EntityTypeName', 'A');\">";
        }
        ...
        hml = hml + "<td width=\"40%\" class=\"td-gridhead\">";
        hml = hml + "<a href=\"\" ";
        if ($("#OrderByField").val() == "EntityTypeCode") {
            if ($("#OrderByDirection").val() == "D") {
                hml = hml + "class=\"sort_descending\" title=\"";
                hml = hml + AlertMsgs.get("DECENDING");
                hml = hml + "\" onclick=\"javascript:return setSort('EntityTypeCode', 'A');\">";
            }
            else {
                hml = hml + "class=\"sort_acending\" title=\"";
                hml = hml + AlertMsgs.get("ACCENDING");
                hml = hml + "\" onclick=\"javascript:return setSort('EntityTypeCode', 'D');\">";
            }
        }
        else {
            hml = hml + "class=\"sort_normal\" onclick=\"javascript:return setSort('EntityTypeCode', 'A');\">";
        }
