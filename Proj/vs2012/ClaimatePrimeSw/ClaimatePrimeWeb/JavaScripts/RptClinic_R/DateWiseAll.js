...
function reImport(pKy) {
    ...
    $.ajax({
        url: (_AppDomainPath + _CtrlrName + "/DateWiseAllAjax/0/0/"),
        type: 'POST',
        data: params,
...
function downloadExcelClinicDate(pky) {
    var wUrl = _AppDomainPath + _CtrlrName + "/DownloadDateExcelAll/0/0/?ky=" + pky;
...
function reImportRefresh() {
    window.location.href = _AppDomainPath + _CtrlrName + "/DateWiseAll/0/0/";
}
