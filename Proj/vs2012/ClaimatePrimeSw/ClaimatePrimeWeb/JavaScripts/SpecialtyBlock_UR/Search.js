    ...
    if (($('#chkHasRec').is(':checked'))) {
        ...
        $.ajax({
            ...
            error: function (req, status, errorObj) {
                ajaxCallError("SpecialtyBlock_UR --> SearchAjaxCall", req, status, errorObj);
            }
        });
    }
...
function searchSuccess(data, status) {
    ...
    if (hml.length == 0) {
        ...
        hml = hml + "<a href=\"\" ";
        if ($("#OrderByField").val() == "SpecialtyName") {
            if ($("#OrderByDirection").val() == "D") {
                hml = hml + "class=\"sort_descending\" title=\"";
                hml = hml + AlertMsgs.get("DECENDING");
                hml = hml + "\" onclick=\"javascript:return setSort('SpecialtyName', 'A');\">";
            }
            else {
                hml = hml + "class=\"sort_acending\" title=\"";
                hml = hml + AlertMsgs.get("ACCENDING");
                hml = hml + "\" onclick=\"javascript:return setSort('SpecialtyName', 'D');\">";
            }
        }
        else {
            hml = hml + "class=\"sort_normal\" onclick=\"javascript:return setSort('SpecialtyName', 'A');\">";
        }
        ...
        hml = hml + "<a href=\"\" ";
        if ($("#OrderByField").val() == "SpecialtyCode") {
            if ($("#OrderByDirection").val() == "D") {
                hml = hml + "class=\"sort_descending\" title=\"";
                hml = hml + AlertMsgs.get("DECENDING");
                hml = hml + "\" onclick=\"javascript:return setSort('SpecialtyCode', 'A');\">";
            }
            else {
                hml = hml + "class=\"sort_acending\" title=\"";
                hml = hml + AlertMsgs.get("ACCENDING");
                hml = hml + "\" onclick=\"javascript:return setSort('SpecialtyCode', 'D');\">";
            }
        }
        else {
            hml = hml + "class=\"sort_normal\" onclick=\"javascript:return setSort('SpecialtyCode', 'A');\">";
        }
