...
function searchSuccess(data, status) {
    ...
    for (i in jsonData) {
        ...
        loadDashTableResult(d.ID);
    }

    return false;
}

function loadDashTableResult(clinicID) {
    var params = "{'pClinicID' : '" + clinicID + "'}";   // if no params need to use "{}"
    $.ajax({
        ...
        dataType: 'json',
        success: function (data, status) {
            SearchSuccessDash(data, status, clinicID);
        },
        error: function (req, status, errorObj) {
...
function SearchSuccessDash(jsonData, status, clinicID) {
    if ($("#divDashSummary" + clinicID).exists()) {
        var retAns = "";
        ...
        loadTotalDashResult(retAns, clinicID);
    }

    return false;
}

function loadTotalDashResult(sumAns, clinicID) {
    var params = "{'pClinicID' : '" + clinicID + "'}";   // if no params need to use "{}"
    $.ajax({
        ...
        dataType: 'json',
        success: function (data, status) {
            SearchSuccessTotalDash(data, status, sumAns, clinicID);
        },
        error: function (req, status, errorObj) {
...
function SearchSuccessTotalDash(jsonData, status, sumAns, clinicID) {
    var retAns = "";
    ...
    $("#divDashSummary" + clinicID).html(sumAns);
    $("#divTotDash" + clinicID).html(retAns);

    $("#divDashSummary" + clinicID).removeAttr("class");
    $("#divDashSummary" + clinicID).attr("class", "dv-table-wrap-summary");

    return false;
}

function showDashPop(status, dCount, pClinicID) {

    $("#aToBrowser").removeAttr("href");
    $("#aToBrowser").attr('href', _AppDomainPath + _CtrlrName + "/Save/0/0/?desc=" + status + "&dayCount=" + dCount + "&pClinicID=" + pClinicID);

    $("#aToExcel").removeAttr("onclick");
    $("#aToExcel").attr("onclick", ("javascript:return saveDashPop('" + _AppDomainPath + _CtrlrName + "/SaveExcel/0/0/?desc=" + status + "&dayCount=" + dCount + "&pClinicID=" + pClinicID + "')"));

    $("#aToPdf").removeAttr("onclick");
    $("#aToPdf").attr("onclick", ("javascript:return saveDashPop('" + _AppDomainPath + _CtrlrName + "/SavePdf/0/0/?desc=" + status + "&dayCount=" + dCount + "&pClinicID=" + pClinicID + "')"));
