...
function expandPnlDiv(ky) {
    ...
    if (divTit != ky) {
        ...
        if ((ky == "Dx")) {
            ...
            if ((_CtrlrName == "AssgnClaims_UR") && ($("#txtDxCount").val() == 0)) {
                alertErrMsg(AlertMsgs.get("ENTER_DX_FIRST"), "DiagName");
                collapsePnlDiv("Dxprimary");
                return expandPnlDiv("Dx");
            }
            else if (($("#PatientVisit_PatientVisitResult_PrimaryClaimDiagnosisID").val().length == 0) || ($("#PatientVisit_PatientVisitResult_PrimaryClaimDiagnosisID").val() == 0)) {
                expandPnlDiv("Dx");
                return collapsePnlDiv("Dx");
            }
