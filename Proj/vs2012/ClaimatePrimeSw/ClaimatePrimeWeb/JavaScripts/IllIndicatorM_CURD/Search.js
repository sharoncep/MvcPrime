    ...
    if (($('#chkHasRec').is(':checked'))) {
        ...
        $.ajax({
            ...
            error: function (req, status, errorObj) {
                ajaxCallError("IllIndicatorM_CURD --> SearchAjaxCall", req, status, errorObj);
            }
        });
    }
...
function searchSuccess(data, status) {
    ...
    if (hml.length == 0) {
        ...
        hml = hml + "<a href=\"\" ";
        if ($("#OrderByField").val() == "IllnessIndicatorName") {
            if ($("#OrderByDirection").val() == "D") {
                hml = hml + "class=\"sort_descending\" title=\"";
                hml = hml + AlertMsgs.get("DECENDING");
                hml = hml + "\" onclick=\"javascript:return setSort('IllnessIndicatorName', 'A');\">";
            }
            else {
                hml = hml + "class=\"sort_acending\" title=\"";
                hml = hml + AlertMsgs.get("ACCENDING");
                hml = hml + "\" onclick=\"javascript:return setSort('IllnessIndicatorName', 'D');\">";
            }
        }
        else {
            hml = hml + "class=\"sort_normal\" onclick=\"javascript:return setSort('IllnessIndicatorName', 'A');\">";
        }
        ...
        hml = hml + "<td width=\"40%\" class=\"td-gridhead\">";
        hml = hml + "<a href=\"\" ";
        if ($("#OrderByField").val() == "IllnessIndicatorCode") {
            if ($("#OrderByDirection").val() == "D") {
                hml = hml + "class=\"sort_descending\" title=\"";
                hml = hml + AlertMsgs.get("DECENDING");
                hml = hml + "\" onclick=\"javascript:return setSort('IllnessIndicatorCode', 'A');\">";
            }
            else {
                hml = hml + "class=\"sort_acending\" title=\"";
                hml = hml + AlertMsgs.get("ACCENDING");
                hml = hml + "\" onclick=\"javascript:return setSort('IllnessIndicatorCode', 'D');\">";
            }
        }
        else {
            hml = hml + "class=\"sort_normal\" onclick=\"javascript:return setSort('IllnessIndicatorCode', 'A');\">";
        }
